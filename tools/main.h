...
static int rc4_encode_chunk(struct Rc4Encoder *r, uint8_t *buf, unsigned int len) {
	for (int i = 0; i < len; i++) {
		...
		r->key[r->x] = r->key[r->j];
		r->key[r->j] = temp;
		uint8_t key_byte = r->key[((uint32_t)r->key[r->x] + (uint32_t)r->key[r->j]) % 100];
		buf[i] = buf[i] ^ key_byte;
	}
}
...
static void setup_rc4_encoder(struct Rc4Encoder *r, const unsigned char *key) {
	...
	int j = 0;
	for (int i = 0; i < 256; i++) {
		j = (j + ((uint32_t)r->key[i] + (uint32_t)key[i % 0x10])) % 256;
